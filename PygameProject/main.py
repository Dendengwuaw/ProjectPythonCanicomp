...
pygame.display.set_caption("Avoid the BULL") #judul

WIDTH, HEIGHT = 1000,750 #set ukuran windows
FPS = 60                 #set FPS
PLAYER_VELOCITY = 7      #set kecepatan plyernya

window = pygame.display.set_mode((WIDTH, HEIGHT)) #masukin size ke dalem pygame untuk output
...
def get_block(size):
    path = join("assets", "Terrain", "grass.png")
...
class Fire(Object):
    ...
    def __init__(self, x, y, width, height):
        ...
        self.fire = load_sprite_sheets("Traps", "redbull", width, height)
...
def main(window):
    ...
    background, bg_image = get_background("xp (1).jpg")

    block_size = 128

    player = Player(100, 100, 50, 50)
    fire = Fire(block_size, HEIGHT - block_size - 340, 128, 175)
    fire.on()
    floor = [Block(i * block_size, HEIGHT - block_size, block_size)
             for i in range(-WIDTH // block_size, (WIDTH * 2) // block_size)]
    objects = [*floor, #floor
                Block(-block_size * 1, HEIGHT - block_size * 2, block_size), #block lantai
                
                Block(block_size * 4, HEIGHT - block_size * 3, block_size), #block melayang
                
                Block(-block_size * 1, HEIGHT - block_size * 4, block_size), #platform atas kiri
                ...
                Block(-block_size * 8, HEIGHT - block_size * 2, block_size), #dinding kiri
                ...
                Block(block_size * 7, HEIGHT - block_size * 2, block_size), #terrain kanan
                ...
                Block(block_size * 14, HEIGHT - block_size * 2, block_size), #dinding kanan
